...
static int uvc_set_vs_commit(const struct device *dev, const struct net_buf *const buf)
{
	 ...
	 * payload sizing, so no format is pushed onto the source here. The
	 * application reads the committed format and frame interval back with
	 * video_get_format() and video_get_frmival() on this device.
	 */
...
/*
 * Adaptation: a bulk-only VideoStreaming interface has no alternate setting to
 * signal the end of a stream. Hosts send CLEAR_FEATURE(ENDPOINT_HALT) to the
 * bulk IN endpoint instead (Windows does, and Linux uvcvideo mimics it). Treat
 * it as the host closing the stream, so that video_get_format() on this device
 * stops reporting a committed stream and the application stops producing
 * frames. The next stream then starts at a frame boundary.
 */
			     ...
			     const bool halted)
{
	...
	if (halted || ep != uvc_get_bulk_in(dev)) {
		return;
	}

	if (atomic_test_and_clear_bit(&data->state, UVC_STATE_STREAM_READY)) {
		LOG_INF("Host stopped the stream");
		atomic_set_bit(&data->state, UVC_STATE_STREAM_RESTART);
	}
}
...
/*
 * Adaptation: the class does not apply the negotiated frame interval to the
 * source (see uvc_set_vs_commit), so report the interval committed by the host
 * to let the application pace its pipeline. If the host did not select one,
 * report the default interval of the selected frame descriptor.
 */
static int uvc_get_frmival(const struct device *dev, enum video_endpoint_id ep,
			   struct video_frmival *const frmival)
{
	struct uvc_data *data = dev->data;
	struct uvc_format_descriptor *format_desc;
	struct uvc_frame_discrete_descriptor *frame_desc;
	...
	if (data->video_frmival.numerator != 0) {
	...
	frmival->numerator = sys_le32_to_cpu(frame_desc->dwDefaultFrameInterval);
	frmival->denominator = NSEC_PER_SEC / 100;
 ...
 * cancel path rather than blocking forever. The same applies to the buffers
 * left if the stream ends while draining.
 *
 * cancel == true (abort): stop the pump, cancel any in-flight bulk IN transfer
 * and return every buffer immediately. A buffer is either still in fifo_in (no
 * USB buffer carries its ownership link yet) or already removed from fifo_in
 * with its final USB buffer in flight (which does carry it); cancelling the
 ...
 *
 * Note that continuation transfers point straight into the video buffer memory
 * (uvc_continue_transfer() is zero-copy), so the cancellation must succeed
 * before any buffer is handed back to the application.
 */
static int uvc_flush(const struct device *dev, enum video_endpoint_id ep, bool cancel)
{
	...
	if (!cancel && atomic_test_bit(&data->state, UVC_STATE_ENABLED) &&
		...
		while (k_fifo_peek_head(&data->fifo_in) != NULL || data->vbuf_offset != 0) {
			if (!atomic_test_bit(&data->state, UVC_STATE_ENABLED) ||
			    !atomic_test_bit(&data->state, UVC_STATE_STREAM_READY)) {
				/* The stream ended: return the rest as for a cancel */
				break;
			}
			k_sleep(K_MSEC(1));
		}

		if (k_fifo_peek_head(&data->fifo_in) == NULL && data->vbuf_offset == 0) {
			return 0;
		}
	}
	 ...
	 * If this fails the controller may still be reading from the video
	 * buffers, so report the error and keep ownership rather than handing
	 * memory that is possibly still in use back to the application.
	 */
	if (atomic_test_bit(&data->state, UVC_STATE_ENABLED)) {
		ret = usbd_ep_dequeue(usbd_class_get_ctx(cfg->c_data), uvc_get_bulk_in(dev));
		if (ret != 0) {
			LOG_ERR("Failed to cancel the pending bulk IN transfers: %d", ret);
			atomic_clear_bit(&data->state, UVC_STATE_PAUSED);
			return ret;
		}
	}
