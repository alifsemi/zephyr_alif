		...
		uint32_t channel, int status)
{
	...
	if (!pdata->record_data) {
		return;
	}

	if (status != 0) {
		LOG_ERR("PDM DMA error: %d", status);
...
static int pdm_initialize(const struct device *dev)
{
	...
	if (cfg->dma_enabled) {
		...
		if (!device_is_ready(cfg->pl330_dev)) {
			LOG_ERR("PL330 DMA device not ready");
			return -ENODEV;
		}
	}
#endif
...
#define PDM_INIT(n)                                                                                \
	...
	static const struct pdm_config dmic_alif_pdm_cfg_##n = {                                   \
		...
		IF_ENABLED(CONFIG_ALIF_PDM_USE_DMA, (                                              \
			.dma_dev = COND_CODE_1(DT_INST_DMAS_HAS_NAME(n, rxdma),                    \
				(DEVICE_DT_GET(DT_INST_DMAS_CTLR_BY_NAME(n, rxdma))),              \
				(NULL)),                                                           \
			.pl330_dev = COND_CODE_1(DT_INST_DMAS_HAS_NAME(n, rxdma),                  \
				(DEVICE_DT_GET(DT_PHANDLE(                                         \
					DT_INST_DMAS_CTLR_BY_NAME(n, rxdma),                       \
					dma_controller))),                                         \
				(NULL)),                                                           \
			.dma_channel = COND_CODE_1(                                                \
				DT_INST_DMAS_HAS_NAME(n, rxdma),                                   \
