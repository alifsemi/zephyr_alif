...
/**
 * @fn		static void alif_pdm_combined_isr(const struct device *dev)
 ...
 * @return	None.
 */
static __maybe_unused void alif_pdm_combined_isr(const struct device *dev)
{
	uintptr_t reg_base = DEVICE_MMIO_GET(dev);
...
#define PDM_INIT(n)                                                                                \
	...
	static void pdm_irq_config_##n(void)                                                       \
	{                                                                                          \
		if (DT_INST_IRQ_HAS_NAME(n, combined_intr)) {                                      \
			IRQ_CONNECT(DT_INST_IRQ_BY_NAME(n, combined_intr, irq),                    \
				DT_INST_IRQ_BY_NAME(n, combined_intr, priority),                   \
				alif_pdm_combined_isr, DEVICE_DT_INST_GET(n), 0);                  \
			irq_enable(DT_INST_IRQ_BY_NAME(n, combined_intr, irq));                    \
		} else {                                                                           \
