...
static uint32_t get_osc_clk_freq(void)
{
	/* TODO check frequency scalings X and Z from HWRM */
	return ALIF_CLOCK_76M8_CLK_FREQ;
}

static uint32_t get_syst_clk_freq(void)
{
	if (sys_read32(CGU_PLL_CLK_SEL) & SYSPLL_SEL) {
		return PLL_CLOCK1_SRC_FREQ;
	} else {
		return get_osc_clk_freq();
	}
}

static uint32_t get_syst_pclk_freq(void)
{
	uint8_t pclk_div_reg = (sys_read32(AON_BUS_CLK_DIV) & 0x3);
	uint8_t pclk_div = (pclk_div_reg & 0x2) ? 4 : ((pclk_div_reg & 0x1) ? 2 : 1);
	uint32_t sys_pll_freq = get_syst_clk_freq();

	return (sys_pll_freq / pclk_div);
}
...
static uint32_t get_he_clock_freq(void)
{
	if (sys_read32(CGU_PLL_CLK_SEL) & SYSPLL_SEL) {
		...
		case 0:
			return (PLL_CLOCK1_SRC_FREQ / 8) * 3;
		case 1:
			return (PLL_CLOCK1_SRC_FREQ / 2);
		case 2:
			return (PLL_CLOCK1_SRC_FREQ / 4) * 3;
		default:
			return PLL_CLOCK1_SRC_FREQ;
		}
	} else {
		uint32_t osc_clk_freq = get_osc_clk_freq();

		switch ((sys_read32(CGU_ESCLK_SEL) >> HE_PLL_DIV_POS) & HE_PLL_DIV_MASK) {
		case 0:
			return osc_clk_freq;
		case 1:
			return osc_clk_freq / 2;
		case 2:
			return osc_clk_freq;
		default:
			return osc_clk_freq / 2;
...
void alif_get_div_reg_info(uint32_t clock_name, uint32_t *mask, uint32_t *pos)
{
	switch (clock_name) {
	case ALIF_CDC200_PIX_SYST_ACLK:
...
static void alif_set_clock_divisor(mem_addr_t reg_addr, uint32_t mask, uint32_t pos, uint32_t value)
{
	uint32_t reg_value;
...
static uint32_t alif_get_clock_divisor(mem_addr_t reg_addr, uint32_t mask, uint32_t pos)
{
