...
static uint32_t alif_get_se_clock(uint32_t *cache, clock_setting_t setting)
{
	...
	if (se_service_clock_setting_get(setting, cache) != 0) {
...
/* Per-source freq helpers: SE query + cache on RTSS, static DT macro otherwise. */
...
{
	ensemble_e1c_clk.sys_clk_cache.extsys1_freq = 0;
}
#endif

#if defined(CONFIG_HAS_ALIF_SE_SERVICES) && IS_ENABLED(CONFIG_PM)
static void ensemble_e1c_clk_pre_device_resume(enum pm_state state)
{
...
static struct pm_notifier ensemble_e1c_clk_pm_notifier = {
	.pre_device_resume = ensemble_e1c_clk_pre_device_resume,
};
...
#if IS_ENABLED(CONFIG_PM_DEVICE)
	((struct ensemble_e1c_clk_data *)dev->data)->cgu_mask = cgu_mask;
#endif

#if defined(CONFIG_HAS_ALIF_SE_SERVICES) && IS_ENABLED(CONFIG_PM)
	pm_notifier_register(&ensemble_e1c_clk_pm_notifier);
#endif
...
#if defined(CONFIG_HAS_ALIF_SE_SERVICES) || IS_ENABLED(CONFIG_PM_DEVICE)
#define ALIF_CLOCK_DATA (&ensemble_e1c_clk)
#else
