...
#define DAC_EN					(1U << 0)
#define DAC_RESET_B				(1U << 27)
#define DAC_HP_MODE_EN				(1U << 18)
#define DAC_MAX_INPUT				(0xFFFU)
#define DAC_IN_BYP_MUX				(1U << 1U)
#define DAC_MAX_BYP_VAL_Msk			(0x3FFCU)
#define DAC_TWOSCOMP_Pos			22U
#define DAC_INPUT_BYP_MUX_Pos			1U
#define DAC_BYP_VAL_Pos				2U
#define DAC_IBIAS_VAL_Pos			23U
#define DAC_CAP_VAL_Pos				14U

#define CMP_CTRL_CMP0_CLKEN			(1U << 0U)
#define VBAT_ANA_REG2_VAL			0x00C00000
...
void dac_analog_config(const struct device *dev)
{
	...
	cmp_reg2_val = (DAC6_VREF_SCALE | DAC6_CONT | DAC6_EN  | DAC12_VREF_CONT |
				ANA_PERIPH_LDO_CONT | ANA_PERIPH_BG_CONT | ADC_VREF_BUF_EN |
				ADC_VREF_CONT | ADC_VREF_BUF_RDIV_EN);

	data = sys_read32(cmp_regs + CMP_REG2_OFFSET);
	data |= cmp_reg2_val;
	sys_write32(data, (cmp_regs + CMP_REG2_OFFSET));
}
...
static inline void dac_set_config(uintptr_t dac, uint8_t input_mux_val, uint8_t conv_input)
{
	...
	data |= ((input_mux_val << DAC_INPUT_BYP_MUX_Pos) | (conv_input << DAC_TWOSCOMP_Pos));
...
void dac_set_output_current(const struct device *dev, uint8_t cur_val)
{
	...
	data = sys_read32(regs + DAC_REG1);
	data |= (cur_val << DAC_IBIAS_VAL_Pos);
	sys_write32(data, (regs + DAC_REG1));
}
...
void dac_set_bypass_input(const struct device *dev, uint32_t bypass_val)
{
	...
	data |= (bypass_val << DAC_BYP_VAL_Pos);
	sys_write32(data, (regs + DAC_REG1));
}
...
bool dac_input_mux_enabled(const struct device *dev)
{
	...
	data = sys_read32(regs + DAC_REG1);
	return (data & DAC_IN_BYP_MUX) ? true : false;
}

static int dac_alif_write_value(const struct device *dev, uint8_t channel, uint32_t input_value)
{
	uintptr_t regs = DEVICE_MMIO_NAMED_GET(dev, dac_reg);
...
static const struct dac_driver_api alif_dac_driver_api = {
	.write_value = dac_alif_write_value,
