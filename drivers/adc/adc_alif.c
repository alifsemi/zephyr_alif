...
static int adc_start_read(const struct device *dev, const struct adc_sequence *sequence)
{
	...
	if (config->dma_dev) {
		...
		if (data->differential) {
			LOG_ERR("DMA mode: differential channel sign-extension not supported");
			return -ENOTSUP;
		}
...
static int adc_suspend(const struct device *dev)
{
	...
	if (config->utimer_timer_base) {
		...
		alif_utimer_stop_counter(global_base, config->utimer_timer_id);
		alif_utimer_disable_timer_clock(global_base, config->utimer_timer_id);
	}
#endif
