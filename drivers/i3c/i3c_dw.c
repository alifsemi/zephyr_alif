...
#define SLV_CHAR_CTRL_DEVICE_ROLE_MASTER   BIT(6)
...
struct dw_i3c_pm_ctx {
	...
	/* flag indicating whether the context has been saved */
	bool valid;
};
...
/**
 * @brief True when the DW I3C IP is idle and safe to disable.
 *
 * Master: PRESENT_STATE.CONTROLLER_IDLE (bit 28) - queues, buffers and
 * controller FSM are all idle. Valid only while CURRENT_MASTER is set.
 *
 * Slave: PRESENT_STATE.CM_TFR_STS (bits 13:8) == 0. CONTROLLER_IDLE and
 * CM_TFR_ST_STS are not valid in target mode.
 */
static bool dw_i3c_hw_is_idle(const struct dw_i3c_config *config)
{
	uint32_t present_state = sys_read32(config->regs + PRESENT_STATE);

	if (present_state & PRESENT_STATE_CURRENT_MASTER) {
		return (present_state & PRESENT_STATE_CONTROLLER_IDLE) != 0;
	}
	return PRESENT_STATE_CM_TFR_STS(present_state) == 0;
}
/**
...
/**
 * @brief The DW I3C controller in preparation for power down.
 *
 * Refuses to suspend with -EBUSY if PRESENT_STATE shows the IP is not
 * idle, so that callers may retry. Otherwise masks all interrupts,
 * snapshots state, disables the controller, gates the input clock, and
 * parks pins in their low-power state.
 *
 * @param dev Pointer to controller device.
 */
static int dw_i3c_suspend(const struct device *dev)
{
	...
	if (!dw_i3c_hw_is_idle(config)) {
		return -EBUSY;
	}

	/*
	 * Mask the NVIC line before gating the input clock. A pending
	 * transfer-complete or IBI IRQ must not run i3c_dw_irq() against
	 * a clock-gated register file (that path takes sem_xfer).
	 */
	...
	if (ret < 0 && ret != -EALREADY && ret != -ENOSYS) {
		/* Clock still on, pins still default — unroll the software disable. */
		goto err_ungate;
	}
	ret = pinctrl_apply_state(config->pcfg, PINCTRL_STATE_SLEEP);
	if (ret < 0 && ret != -ENOENT) {
		/*
		 * Clock is already off and pins may be in sleep/GPIO. Full
		 * resume puts default pinctrl, clock, SCL timings, DAT and
		 * ENABLE back; err_ungate cannot do that.
		 */
		(void)dw_i3c_resume(dev);
		return ret;
	}
	return 0;
...
#define DEFINE_DEVICE_FN(n)                                                                        \
	...
	static const struct dw_i3c_config dw_i3c_cfg_##n = {                                       \
		...
		IF_ENABLED(CONFIG_PM_DEVICE, (.irqn = DT_INST_IRQN(n),))                           \
