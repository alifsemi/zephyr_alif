...
#define SLV_INTR_REQ_IBI_STS_ACK        0x01
#define SLV_INTR_REQ_IBI_STS_NATMPTED   0x03
...
static int dw_i3c_target_ibi_raise_tir(const struct device *dev, struct i3c_ibi *request)
{
	...
	case SLV_INTR_REQ_IBI_STS_ACK:
		LOG_DBG("%s: Controller ACKed IBI TIR", dev->name);
		return 0;
	case SLV_INTR_REQ_IBI_STS_NATMPTED:
		LOG_ERR("%s: Controller NATMPTED IBI TIR", dev->name);
		return -EAGAIN;
	default:
		return -EIO;
	}
