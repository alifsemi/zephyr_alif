					     ...
					     uint8_t *rdata, uint32_t length)
{
	...
	bool                                      octal_status_read;
	int                                       ret;

	/* Read status in Octal DDR mode */
	if (data->octal_ddr && (addr_len == 0U) && (length == 1U)) {
		octal_status_read = true;
	} else {
		octal_status_read = false;
	}
	...
	if (octal_status_read) {
		/* Octal DDR mode: return the low byte. */
...
static int flash_mspi_is25xX0xx_init(const struct device *flash)
{
	...
	if (cfg->tar_dev_cfg.addr_length == 4 && !data->octal_ddr) {
