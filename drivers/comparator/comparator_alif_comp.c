...
struct cmp_data {
	...
	DEVICE_MMIO_NAMED_RAM(dac6_vref);
...
#if defined(CONFIG_SOC_SERIES_E7) || defined(CONFIG_SOC_SERIES_E3)
#define CMP_WINDOW_CONTROL_ENABLE (3U)
#define CMP_INT_STATUS_MASK       (1U)
#elif defined(CONFIG_SOC_SERIES_E1C) || defined(CONFIG_SOC_SERIES_B1)
#define CMP_WINDOW_CONTROL_ENABLE (1U)
#define CMP_INT_STATUS_MASK       (3U)
#elif defined(CONFIG_SOC_SERIES_E8) || defined(CONFIG_SOC_SERIES_E4)
#define CMP_WINDOW_CONTROL_ENABLE (3U)
#define CMP_INT_STATUS_MASK       (3U)
#endif
...
{
	sys_write32(presclar, cmp + CMP_PRESCALER_CTRL);
}
...
void cmp_irq_handler(const struct device *dev)
{
	...
	if (config->drv_inst != CMP_INSTANCE_LP) {
		...
		if (int_status == CMP_FILTER_EVENTS_CLEAR_ALL) {
			sys_write32(CMP_FILTER_EVENTS_CLEAR_ALL, regs + CMP_INTERRUPT_STATUS);
		}
	}

	if (data->callback) {
		data->callback(dev, NULL);
	}
}
...
#define CMP_ALIF_INIT(inst)                                                                        \
	...
	const struct cmp_config config_##inst = {                                                  \
		...
		.filter_taps = DT_INST_PROP(inst, filter_taps),                                    \
