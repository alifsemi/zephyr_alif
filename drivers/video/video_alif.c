...
static int alif_cam_suspend(const struct device *dev)
{
	...
	ret = clock_control_off(config->clk_dev, config->cid);
	if (ret) {
		LOG_ERR("Failed to disable CPI clock during suspend: %d", ret);
		return ret;
	}

	if (config->pixclk) {
		ret = clock_control_off(config->clk_dev, config->pixclk);
		if (ret) {
			LOG_ERR("Failed to disable pixclk during suspend: %d", ret);
			return ret;
		}
	}
...
#define CAM_GET_CLK(i)                                                         \
	IF_ENABLED(DT_INST_NODE_HAS_PROP(i, clocks),                               \
		...
		.cid = (clock_control_subsys_t)DT_INST_CLOCKS_CELL_BY_NAME(i,         \
			cam_clk, clkid),                                                  \
		.pixclk = COND_CODE_1(DT_INST_CLOCKS_HAS_NAME(i, pix_clk),             \
			((clock_control_subsys_t)DT_INST_CLOCKS_CELL_BY_NAME(i,        \
				pix_clk, clkid)),                                         \
