...
static int csi2_dw_validate_data(const struct device *dev)
{
	...
	if (config->ipi_mode == CSI2_IPI_MODE_TIMINGS_CAM) {
		float time_PPI_ns = (4.0 / phy->pll_fin) * 1000000000;
		float time_IPI_ns = (1.0f / pixclock) * 1000000000;
		float pkt2pkt_time_ns = data->pkt2pkt_time.time_ns;
		uint32_t bytes_to_transmit = DIV_ROUND_UP(timing->hact * bpp, 8);
		...
		tmp_f = (pkt2pkt_time_ns +
			 (DIV_ROUND_UP(2U * CSI2_SHORT_PKT_BYTES, phy->num_lanes) + 2U) *
		...
		tmp_f = (pkt2pkt_time_ns +
			 (DIV_ROUND_UP(CSI2_LONG_PKT_BYTES + bytes_to_transmit, phy->num_lanes) *
			  time_PPI_ns / CSI2_BYTES_PER_HS_CLK)) /
			time_IPI_ns;
		...
		if (CSI2_IPI_FIFO_DEPTH < min_memory_depth) {
			LOG_ERR("FIFO memory inadequate!");
			return -EINVAL;
		}
		timing->hact = 0;
	}
		...
		.phy[1] =  {                                                                       \
			...
			.pll_fin = COND_CODE_1(DT_NODE_HAS_PROP(REMOTE_EP(i, 1, 0),                \
					...
					(DT_INST_PROP(i, rx_ddr_clk2))),                           \
                                                                                                   ...
                                                                                                   \
		.pkt2pkt_time = {                                                                  \
			.line_sync_pkt_enable = DT_INST_PROP_OR(i, sync_pkt_enable, 0),            \
