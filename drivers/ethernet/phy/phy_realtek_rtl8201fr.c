...
#define LOG_MODULE_NAME phy_mc_rtl8021fr
#define LOG_LEVEL CONFIG_PHY_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(LOG_MODULE_NAME);

struct phy_rtl8021fr_config {
	uint8_t addr;
...
{
	const struct phy_rtl8021fr_config *config = dev->config;
	int ret;
...
{
	const struct phy_rtl8021fr_config *config = dev->config;
	struct phy_rtl8021fr_data *data = dev->data;
	uint32_t bmsr = 0;
	uint32_t anar = 0;
	uint32_t anlpar = 0;
	int ret;
...
static int phy_mc_rtl8021fr_init(const struct device *dev)
{
	const struct phy_rtl8021fr_config *config = dev->config;
	uint32_t ret = 0;
	uint32_t val;
	...
	phy_rtl8201fr_read(dev, MII_PHYID1R, &val);
	...
	phy_rtl8201fr_write(dev, MII_BMCR, bmcr);
	if (ret) {
		LOG_ERR("Error writing phy (%d) basic control register", config->addr);
		return ret;
	}
...
#define REALTEK_INIT(n)								\
	static const struct phy_rtl8021fr_config mc_rtl8021fr_##n##_config = {	\
		...
		.mode = DT_INST_ENUM_IDX(n, phy_mode),				\
										...
										\
	static struct phy_rtl8021fr_data mc_rtl8021fr_##n##_data;		\
										\
	DEVICE_DT_INST_DEFINE(n, &phy_mc_rtl8021fr_init, NULL,			\
			&mc_rtl8021fr_##n##_data, &mc_rtl8021fr_##n##_config,	\
