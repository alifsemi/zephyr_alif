...
/*
 * NOTE (LVGL 9.5.0 backport): revert when the Zephyr base is updated. This fork
 * predates <zephyr/debug/cpu_load.h>; it is only needed for cpu_load_get(),
 * which is already guarded by CONFIG_CPU_LOAD below, so gate the include too.
 */
