...
#define REGION_OSPI_XIP_ATTR(rbar_attr, mair_attr, base, size) \
{\
	.rbar = (rbar_attr) | NON_SHAREABLE_Msk, \
	.mair_idx = (mair_attr), \
	.r_limit = REGION_LIMIT_ADDR(base, size),  \
}

#if OSPI_RAM_NODE_ENABLED
#define OSPI_RAM_XIP_NODE		DT_PARENT(OSPI_RAM_NODE)
#define OSPI_RAM_XIP_BASE		DT_REG_ADDR_BY_NAME(OSPI_RAM_XIP_NODE, xip)
#define OSPI_RAM_XIP_SIZE		DT_PROP_BY_IDX(OSPI_RAM_NODE, xip_config, 2)
...
#define OSPI_RAM_XIP_MAIR_INDEX		MPU_MAIR_INDEX_DEVICE
#else
#define OSPI_RAM_XIP_MAIR_INDEX		MPU_MAIR_INDEX_SRAM
#endif
#endif

#if OSPI_FLASH_NODE_ENABLED
#define OSPI_FLASH_XIP_NODE		DT_PARENT(OSPI_FLASH_NODE)
#define OSPI_FLASH_XIP_BASE		DT_REG_ADDR_BY_NAME(OSPI_FLASH_XIP_NODE, xip)
#define OSPI_FLASH_XIP_SIZE		DT_PROP_BY_IDX(OSPI_FLASH_NODE, xip_config, 2)
#endif
...
static const struct arm_mpu_region mpu_regions[] = {
	...
	/* OSPI0 and OSPI1 controller registers */
	MPU_REGION_ENTRY("OSPI_CTRL", ALIF_ENSEMBLE_OSPI_REG,
			 REGION_DEVICE_ATTR(ALIF_ENSEMBLE_OSPI_REG, ALIF_ENSEMBLE_OSPI_SIZE)),
#endif
#if OSPI_RAM_NODE_ENABLED
	/* HyperRAM XIP region */
	MPU_REGION_ENTRY("OSPI_RAM_XIP", OSPI_RAM_XIP_BASE,
			 REGION_OSPI_XIP_ATTR(NOT_EXEC | P_RW_U_NA_Msk,
					      OSPI_RAM_XIP_MAIR_INDEX,
					      OSPI_RAM_XIP_BASE, OSPI_RAM_XIP_SIZE)),
#endif
#if OSPI_FLASH_NODE_ENABLED
	/* Flash XIP region */
	MPU_REGION_ENTRY("OSPI_FLASH_XIP", OSPI_FLASH_XIP_BASE,
			 REGION_OSPI_XIP_ATTR(RO_Msk, MPU_MAIR_INDEX_FLASH,
					      OSPI_FLASH_XIP_BASE, OSPI_FLASH_XIP_SIZE)),
#endif
};
