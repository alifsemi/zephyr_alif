...
class AlifImageBinaryRunner(ZephyrBinaryRunner):
    ...
    def flash(self, **kwargs):
        ...
        fls_base = self.build_conf.get('CONFIG_FLASH_BASE_ADDRESS')
        ...
        old_cwd = os.getcwd()
        build_dir = os.path.abspath(self.cfg.build_dir)
        try:
            ...
            if not self.prepare_json(self.logger, build_core, fls_addr, fls_size,
                                     ...
                                     build_dir=build_dir,
                                     flash_base=fls_base):
                raise RuntimeError("Failed to prepare Alif ToC configuration")
    ...
    def _parse_build_dts(cls, logger, build_dir=None):
        ...
        if MISSING_REQUIREMENTS:
            raise RuntimeError(
                ...
                "into RAM; install it with 'python -m pip install fdt'")
    ...
    def prepare_json(cls, logger, build_core, fls_addr, fls_size,
        ...
        try:
            ...
            logger.error("Invalid JSON format in %s: %s", cfg_ip_path, err)
            return False
        except OSError as err:
        ...
        image_names = [(local_node_name, 'zephyr.bin')]
        image_names.extend(
            (node_name, os.path.basename(binary))
            for node_name, binary, _address, _mram_boot in cpu_args
            if binary is not None and node_name != local_node_name
        )
        if (build_core == "a32" or a32_bin is not None) and bl32_bin is not None:
            image_names.append(("BOOTLOAD", os.path.basename(bl32_bin)))
        if not cls._validate_unique_image_names(logger, image_names):
            return False
        ...
        if build_core == "a32" or a32_bin is not None:
            try:
                cls._validate_a32_bootload_args(bl32_bin,
                                                bl32_flash_address)
            except ValueError as err:
                logger.error("%s", err)
                return False
    ...
    @staticmethod
    def _validate_unique_image_names(logger, image_names):
        """Reject image names which collide in the flat SE tools directory."""
        destinations = {}
        for node_name, image_name in image_names:
            destination = image_name.casefold()
            if destination in destinations:
                logger.error(
                    "%s and %s images have the same destination name %s",
                    destinations[destination], node_name, image_name)
                return False
            destinations[destination] = node_name
        return True
    ...
    def _apply_a32_bootload_node(cls, json_data, logger, bl32_bin=None,
                                 ...
                                 mram_ranges=None, flash_base=None):
        ...
        try:
            cls._add_mram_range("BOOTLOAD", address, address + bin_size,
                                fls_size, mram_ranges or [], flash_base)
        except ValueError as err:
            logger.error("%s", err)
            return False
    ...
    @classmethod
    def _mram_limit(cls, fls_size, flash_base=None):
        if fls_size is None:
            return None
        if flash_base is None or flash_base < cls.mram_base_addr:
            base = cls.mram_base_addr
        else:
            base = flash_base
        return base + (fls_size * 1024)
    ...
    def _validate_mram_boundary(cls, node_name, start, end, fls_size,
        ...
        mram_limit = cls._mram_limit(fls_size, flash_base)
    ...
    def _add_mram_range(cls, node_name, start, end, fls_size, mram_ranges,
                        flash_base=None):
        cls._validate_mram_boundary(node_name, start, end, fls_size, flash_base)
    ...
    def _apply_cpu_arg_image(cls, logger, json_data, node_name, binary, address,
        ...
        if mram_boot:
            try:
                cls._add_mram_range(node_name, address, address + bin_size,
                                    fls_size, mram_ranges, flash_base)
            except ValueError as err:
                logger.error("%s", err)
                return False
    ...
    def _apply_local_build_image(cls, logger, json_data, node_name, fls_addr,
                                 fls_size, build_dir=None, flash_base=None):
        if node_name not in json_data:
        ...
        logger.error("Unsupported address base 0x%x to write", fls_addr)
        return False
